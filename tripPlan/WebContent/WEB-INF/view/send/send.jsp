	...
	<div class="container">
		<form action="senddelete.do" method="post">
			<div class="btn-group">
				...
				<a href="receive.do" class="btn btn-info" role="button">받은 쪽지&nbsp;&nbsp;${r_count}</a> 
				<a href="send.do" class="btn btn-info send" role="button">보낸 쪽지&nbsp;&nbsp;${count}</a>
			</div>
							...
							<c:forEach var="send" items="${sendList}">
								<tr height="30">
									...
									<c:if test="${send.sm_check == 0}">
										<th style="text-align: center; position: absolute; right: 40;">
										<i class="fa fa-envelope-o"></i></th>
									</c:if>
									<c:if test="${send.sm_check == 1}">
										<th style="text-align: center; position: absolute; right: 40;">
										<i class="fa fa-envelope-open"></i></th>
									</c:if>
								</tr>
							</c:forEach>
						</table>
